...
def test_query1_initial():
    ...
    print("\n--- Chi tiết kết quả ---")
    print(f"Hoàn thành: {result['is_complete']}")
    print(f"Thông tin đã thu thập: {len(result['collected_info'])} trường")
    print(f"Số câu hỏi còn lại: {len(result['questions'])}")
    print(f"Trạng thái từng trường:")
    for field, status in result['info_status'].items():
        status_text = "✓ Đã có" if status == 1 else ("◐ Một phần" if status == 0.5 else "✗ Chưa có")
        print(f"  - {field}: {status_text}")
...
def test_query1_followup():
    ...
    payload = {
        "query": "Tên tôi là Budi Santoso, sinh năm 1990, có 2 người",
        "collected_info": {
